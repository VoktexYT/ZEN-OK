...
# Installation yay si nécessaire pour AUR
if [ -f "$BACKUP_DIR/packages_aur.txt" ] && [ -s "$BACKUP_DIR/packages_aur.txt" ]; then
    if ! command -v yay &>/dev/null; then
        print_info "Installation de yay pour les packages AUR..."
...
print_header "ÉTAPE 2/6 - Restauration des configurations"
echo ""
...
if [ -d "$BACKUP_DIR/configs" ]; then
    ...
    # GTK
    if [ -d "$BACKUP_DIR/configs/gtk-3.0" ]; then
        mkdir -p "$HOME/.config/gtk-3.0"
        cp -r "$BACKUP_DIR/configs/gtk-3.0/"* "$HOME/.config/gtk-3.0/"
    fi
    if [ -d "$BACKUP_DIR/configs/gtk-2.0" ]; then
        mkdir -p "$HOME/.config/gtk-2.0"
        cp -r "$BACKUP_DIR/configs/gtk-2.0/"* "$HOME/.config/gtk-2.0/"
    fi
fi
...
print_header "ÉTAPE 3/6 - Restauration des dotfiles"
echo ""
...
[ -f "$BACKUP_DIR/configs/.tmux.conf" ] && cp "$BACKUP_DIR/configs/.tmux.conf" "$HOME/" && print_success ".tmux.conf"
...
print_header "ÉTAPE 4/6 - Restauration des fonts"
echo ""
...
print_header "ÉTAPE 5/6 - Restauration des scripts"
echo ""
...
# 6. Services runit
print_header "ÉTAPE 6/6 - Configuration des services"
echo ""

print_info "Vérifie que NetworkManager est activé..."
